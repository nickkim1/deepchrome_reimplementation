...
def testing(model, cell_type, num_epochs, num_genes, training_dataloader, device, criterion, optimizer, n_total_steps, scheduler, model_path,  vt_dataloader):
    
    model = ConvNet(width=100, num_filters=50, filter_size=10, pool_size=5, hidden_layer_units={"first": 625, "second": 125})
    vt_dataset = HistoneDataset(file_path='toy_data_4/E001/valid.csv', num_cols=8, num_windows=100)
    vt_dataloader = DataLoader(dataset=vt_dataset, batch_size=100, shuffle=False, drop_last=False)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (samples, labels) in enumerate(training_dataloader):
            ...
            loss.backward() 
            optimizer.step() 
            if (i+1) % num_genes == 0: # where the i term above is used in for loop
                print (f'Epoch [{epoch+1}/{num_epochs}], Step [{i+1}/{n_total_steps}], Loss: {loss.item():.4f}')
    ...
    return target_scores, predicted_scores, cm_scores, training_time_list, evaluation_time_list, log
...
def main():
    ...
    for index, dirs in enumerate(lowest_dirs): 
        ...
        target_scores, predicted_scores, cm_scores, training_time_list, evaluation_time_list, log = testing(model=model, cell_type=cell_type, num_epochs=num_epochs, num_genes=num_genes, training_dataloader=training_dataloader, device=device, criterion=criterion, optimizer=optimizer, n_total_steps=n_total_steps, scheduler=scheduler, model_path=model_path, vt_dataloader=vt_dataloader)
